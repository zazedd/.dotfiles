...
local brightness_text = wibox.widget({
	...
	markup = helpers.colorize_text("100%", beautiful.text),
...
local brightness_icon = wibox.widget({
	widget = wibox.widget.textbox,
	markup = helpers.colorize_text("󰖨", beautiful.fg_color),
...
function update_value_of_brightness()
	awful.spawn.easy_async_with_shell(
		...
		function(stdout)
			...
			brightness_text.markup = helpers.colorize_text(value .. "%", beautiful.text)
		end
	)
end
...
local function update_widget_according_to_vol_muted(_)
	...
	volume_icon.font = "JetBrains Mono Nerd Font 20"
end
...
local function toggle_mute()
	if not vol_muted then
		awful.spawn("amixer -D pipewire set Master mute", false)
		vol_muted = true
		update_widget_according_to_vol_muted(vol_muted)
		awesome.emit_signal("volume::muted", vol_muted)
	else
		awful.spawn("amixer -D pipewire set Master unmute", false)
		vol_muted = false
		update_widget_according_to_vol_muted(vol_muted)
		awesome.emit_signal("volume::muted", vol_muted)
	end
...
awesome.connect_signal("volume::value", function(value)
	...
	volume_text.markup = helpers.colorize_text(value .. "%", beautiful.text)
end)

awesome.connect_signal("volume::changed", function(value)
	volume.value = volume.value + value
	volume_text.markup = helpers.colorize_text(volume.value .. "%", beautiful.text)
end)

volume:connect_signal("property::value", function(_, new_value)
	volume_text.markup = helpers.colorize_text(new_value .. "%", beautiful.text)
	volume.value = new_value
	awful.spawn("amixer -D pipewire set Master " .. new_value .. "%", false)
	awesome.emit_signal("volume::value", new_value)
end)

volume_icon:buttons(gears.table.join(awful.button({}, 1, function()
	toggle_mute()
end)))
		...
		{
			{
				{
					widget = wibox.widget.textbox,
					markup = helpers.colorize_text("Controls", beautiful.text .. "4D"),
					font = beautiful.font_var .. "10",
					align = "left",
					valign = "center",
				},
